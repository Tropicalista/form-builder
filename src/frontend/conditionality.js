...
function evaluate () {
  const elements = document.querySelectorAll('.formello-form [data-show-if], .formello-form [data-hide-if]');
  [].forEach.call(elements, toggleElement)
}

// re-evaluate conditional elements for change events on forms
function handleInputEvent (evt) {
  if (!evt.target || !evt.target.form || evt.target.form.className.indexOf('formello-form') < 0) {
    return
  }
